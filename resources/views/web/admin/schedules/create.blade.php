...
@section('title', 'إضافة نشاط للجدول الأسبوعي')

@section('content')
<div class="container-fluid">
     <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">إضافة نشاط للجدول الأسبوعي</h1>
         <a href="{{ route('admin.schedules.index', ['class_id' => $selectedClassId]) }}" class="btn btn-outline-secondary">
             ...
             <form action="{{ route('admin.schedules.store') }}" method="POST">
                @csrf
                 ...
                 <div class="mb-3">
                    ...
                    <select class="form-select @error('class_id') is-invalid @enderror" id="class_id" name="class_id" required>
                        <option value="" disabled {{ old('class_id', $selectedClassId) ? '' : 'selected' }}>-- اختر الفصل --</option>
                ...
                {{-- حقل يوم الأسبوع --}}
                 <div class="mb-3">
                    ...
                    <select class="form-select @error('day_of_week') is-invalid @enderror" id="day_of_week" name="day_of_week" required>
                         <option value="" disabled {{ old('day_of_week') ? '' : 'selected' }}>-- اختر اليوم --</option>
                ...
                <div class="row">
                 ...
                 <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('activity_description') is-invalid @enderror" id="activity_description" name="activity_description" value="{{ old('activity_description') }}" placeholder="مثال: قراءة قصة, ألعاب حركية, فنون..." required>
                    @error('activity_description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>


                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ route('admin.schedules.index', ['class_id' => $selectedClassId]) }}" class="btn btn-outline-secondary me-2">إلغاء</a>
                    <button type="submit" class="btn btn-primary">
                        <i data-feather="save" class="me-1"></i> حفظ النشاط
                    </button>
                </div>
            </form>
        </div>
