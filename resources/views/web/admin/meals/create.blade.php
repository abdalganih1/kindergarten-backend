...
<div class="container-fluid">
     <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">إضافة وجبة جديدة</h1>
         <a href="{{ route('admin.meals.index', ['date' => $defaultDate]) }}" class="btn btn-outline-secondary">
        ...
        <div class="card-body">
             <form action="{{ route('admin.meals.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('meal_date') is-invalid @enderror" id="meal_date" name="meal_date" value="{{ old('meal_date', $defaultDate) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('meal_type') is-invalid @enderror" id="meal_type" name="meal_type" required>
                        <option value="" disabled {{ old('meal_type') ? '' : 'selected' }}>-- اختر النوع --</option>
                        @foreach($mealTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('meal_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                 ...
                 <div class="mb-3">
                    ...
                    <textarea class="form-control @error('menu_description') is-invalid @enderror" id="menu_description" name="menu_description" rows="5" required>{{ old('menu_description') }}</textarea>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ route('admin.meals.index', ['date' => $defaultDate]) }}" class="btn btn-outline-secondary me-2">إلغاء</a>
                    <button type="submit" class="btn btn-primary">
                        <i data-feather="save" class="me-1"></i> حفظ الوجبة
                    </button>
                </div>
            </form>
        </div>
