...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">تعديل وجبة ليوم: {{ \Carbon\Carbon::parse($dailyMeal->meal_date)->format('Y-m-d') }}</h1>
        <a href="{{ route('admin.meals.index', ['date' => $dailyMeal->meal_date]) }}" class="btn btn-outline-secondary">
     ...
     <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.meals.update', ['dailyMeal' => $dailyMeal]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('meal_date') is-invalid @enderror" id="meal_date" name="meal_date" value="{{ old('meal_date', $dailyMeal->meal_date) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('meal_type') is-invalid @enderror" id="meal_type" name="meal_type" required>
                        ...
                        @foreach($mealTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('meal_type', $dailyMeal->meal_type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                 ...
                 <div class="mb-3">
                    ...
                    <textarea class="form-control @error('menu_description') is-invalid @enderror" id="menu_description" name="menu_description" rows="5" required>{{ old('menu_description', $dailyMeal->menu_description) }}</textarea>
                    ...
                    <select class="form-select @error('class_id') is-invalid @enderror" id="class_id" name="class_id">
                        <option value="">-- وجبة عامة (لكل الفصول) --</option>
                         @foreach($classes as $id => $name)
                            <option value="{{ $id }}" {{ old('class_id', $dailyMeal->class_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ route('admin.meals.index', ['date' => $dailyMeal->meal_date]) }}" class="btn btn-outline-secondary me-2">إلغاء</a>
