...
@section('header-buttons')
    <a href="{{ route('admin.children.edit', $child) }}" class="btn btn-sm btn-warning me-2">
        <i data-feather="edit-2" class="me-1"></i> تعديل الملف
    </a>
    <form action="{{ route('admin.children.destroy', $child) }}" method="POST" class="d-inline" onsubmit="return confirm('تحذير! سيتم حذف ملف الطفل وجميع السجلات المرتبطة به. هل أنت متأكد؟');">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-sm btn-danger me-2">
            <i data-feather="trash-2" class="me-1"></i> حذف الملف
        </button>
    </form>
        ...
        <div class="card-body">
            <div class="tab-content" id="childDetailsTabContent">
                    ...
                    <h5 class="mb-3">سجل الحضور والغياب (الأحدث أولاً)</h5>
                     <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-sm table-bordered table-striped">
                            <thead>
                                <tr><th>التاريخ</th><th>الحالة</th><th>وقت الدخول</th><th>وقت الخروج</th><th>ملاحظات</th><th>سُجّل بواسطة</th></tr>
                            </thead>
                            <tbody>
                                @forelse($child->attendances()->latest('attendance_date')->get() as $att)
                                    <tr>
                ...
                <div class="tab-pane fade" id="health-content" role="tabpanel" aria-labelledby="health-tab">
                    ...
                    {{-- يمكنك إضافة زر لإضافة سجل صحي جديد هنا --}}
                    <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                         <table class="table table-sm table-bordered table-striped">
                             <thead>
                                <tr><th>التاريخ</th><th>النوع</th><th>التفاصيل</th><th>تاريخ الاستحقاق القادم</th><th>أُدخل بواسطة</th></tr>
                            </thead>
                             <tbody>
                                 @forelse($child->healthRecords()->latest('record_date')->get() as $rec)
                                    <tr>
                ...
                <div class="tab-pane fade" id="events-content" role="tabpanel" aria-labelledby="events-tab">
                     ...
                     <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                         <table class="table table-sm table-bordered table-striped">
                             <thead>
                                <tr><th>الفعالية</th><th>تاريخ الفعالية</th><th>تاريخ التسجيل</th><th>موافقة ولي الأمر</th></tr>
                            </thead>
                             <tbody>
...
@push('scripts')
<script>
    // لتفعيل التبويب الأول عند تحميل الصفحة (اختياري)
    var firstTabEl = document.querySelector('#childDetailsTab li:first-child button')
    if(firstTabEl){
        var tab = new bootstrap.Tab(firstTabEl)
        // tab.show() // يمكنك تفعيل هذا إذا أردت التبويب الأول يكون نشطًا دائمًا عند التحميل
    }
</script>
@endpush