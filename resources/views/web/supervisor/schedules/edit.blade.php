...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">تعديل نشاط في الجدول</h1>
         {{-- رابط العودة يشير لمسار المشرف --}}
         <a href="{{ route('supervisor.schedules.index', ['class_id' => $weeklySchedule->class_id, 'day_of_week' => $weeklySchedule->day_of_week]) }}" class="btn btn-outline-secondary">
     ...
     <div class="card">
         <div class="card-body">
            {{-- رابط الإرسال يشير لمسار المشرف --}}
             <form action="{{ route('supervisor.schedules.update', $weeklySchedule) }}" method="POST">
                @csrf
                @method('PUT')

                {{-- حقل الفصل الدراسي (فصول المشرف فقط) --}}
                 <div class="mb-3">
                    ...
                    <select class="form-select @error('class_id') is-invalid @enderror" id="class_id" name="class_id" required>
                        @if(isset($classes) && !$classes->isEmpty())
                            ...
                            @endforeach
                        @else
                             <option value="{{ $weeklySchedule->class_id }}" selected>{{ $weeklySchedule->kindergartenClass->class_name ?? 'الفصل الحالي' }} (لا يمكنك تغيير الفصل)</option>
                        @endif
                    </select>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="time" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" value="{{ old('start_time', $weeklySchedule->start_time ? \Carbon\Carbon::parse($weeklySchedule->start_time)->format('H:i') : '') }}" required>
                        @error('start_time') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                     {{-- حقل وقت الانتهاء --}}
                    <div class="col-md-6 mb-3">
                         <label for="end_time" class="form-label">وقت الانتهاء <span class="text-danger">*</span></label>
                        <input type="time" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time', $weeklySchedule->end_time ? \Carbon\Carbon::parse($weeklySchedule->end_time)->format('H:i') : '') }}" required>
                ...
                <div class="d-flex justify-content-end mt-4">
                     {{-- رابط الإلغاء يشير لمسار المشرف --}}
                     <a href="{{ route('supervisor.schedules.index', ['class_id' => $weeklySchedule->class_id, 'day_of_week' => $weeklySchedule->day_of_week]) }}" class="btn btn-outline-secondary me-2">إلغاء</a>
